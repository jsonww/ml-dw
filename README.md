...
- 将原始数据文件直接读入，不作任何修改，仅作备份
- 由于数据文件中的字段可能包含半角逗号“,”，与 csv 文件默认分隔符相同，为避免冲突，使用 OpenCSVSerde 加载数据文件，ci'shi读入数据字段的类型皆为字符串类型



### DWD 层 / DIM 层

DWD 层和 DIM 层分别抽取数据文件中的具体业务过程和可单独使用的维度信息单独成表。
...
- DWS 层在 DWD 层的业务过程基础上，以年为时间粒度，分别对用户、电影进行轻度汇总
- DWT 层在 DWS 层的轻度汇总基础上，分别以年度、用户、电影、电影类型进行全量汇总



### ADS 层

对数据仪表板中预期使用频率较高的可视化需求数据进行提取：

1. 历年累计评级/标签数量变化
1. 历年的年度平均评级和累计平均评级变化
1. 各电影类型数量
2. 各电影类型的评级与标签数量
3. 评级数量前十的电影及其评级相关信息
4. 每年评级/标签用户数量变化
4. 每年第一次使用和最近一次使用的用户数量
6. 评级数量前 100 用户及其评级相关信息
...
|实体|命名规范|
|-|-|
| 数据库 | `ml_{ods|dwd |dim|dws|dwt|ads}` |
| ODS 层数据表 | `ods_<数据来源>` |
| DWD 层数据表 | `dwd_<业务>` |
| DIM 层数据表 | `dim_<维度>` |
...
注：由于原始数据文件中的某些字段存在半角逗号“,”，与数据文件本身格式 csv 的分隔符“,”相同，为避免数据加载到 hive 时分割错误，使用 OpenCSV 解析数据文件。
...
2. dim_movies_genres
   ...
   FROM
       ml_ods.ods_movies AS m
   LATERAL VIEW
       EXPLODE(SPLIT(m.genres, '\\|')) gen_tmp AS gen;  -- 通过 shell 执行 ‘|’ 需要转义
   ```
